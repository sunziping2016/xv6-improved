...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    ...
    // modified bmap, adding more single-indirect block
    uint blockoffset = bn % PINDIRECT;
    uint blockindex = bn / PINDIRECT;
    if ((addr = ip->addrs[NDIRECT + blockindex]) == 0)
      ip->addrs[NDIRECT + blockindex] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[blockoffset]) == 0){
      a[blockoffset] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    return addr;
